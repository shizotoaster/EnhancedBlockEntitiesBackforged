...
public interface ModelSelector {
    ...
    ModelSelector SHULKER_BOX = (view, state, pos, rand, ctx) -> {
        if(view.getBlockEntity(pos) instanceof ShulkerBoxBlockEntity) {
            ShulkerBoxBlockEntity be = (ShulkerBoxBlockEntity) view.getBlockEntity(pos);
            if(be.getAnimationProgress(1) > 0) return 1;
        }
        return 0;
    };
